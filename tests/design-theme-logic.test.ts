...
describe("design theme logic", () => {
  it("normalizes only supported design themes", () => {
    ...
    expect(normalizeDesignTheme("retro")).toBe("neon");
    expect(normalizeDesignTheme("obsidian")).toBe("neon");
    expect(normalizeDesignTheme("borealis")).toBe("neon");
    expect(normalizeDesignTheme("rose")).toBe("neon");
  ...
  it("maps distinct effects per design theme", () => {
    ...
    const neonDark = resolveDesignEffects("neon", "dark");
    expect(neonDark.glowPrimary).toContain("rgba");
  });
