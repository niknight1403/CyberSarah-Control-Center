...
export function CyberAgentCard({ name, role, status, metric, onPress }: CyberAgentCardProps) {
  ...
  const statusColor = status === "success" ? glass.glassPalette.green : status === "running" ? glass.glassPalette.cyan : status === "error" ? glass.glassPalette.red : status === "warn" ? glass.glassPalette.amber : glass.glassSurface.textSecondary;
  return (
    <Pressable onPressIn={handlePressIn} onPressOut={handlePressOut} onPress={onPress} accessibilityRole="button">
      <Animated.View style={[styles.card, { shadowColor: statusColor }, animatedStyle]}>
        ...
        <View style={styles.footer}><Text style={[styles.statusText, { color: statusColor }]}>{status.toUpperCase()}{metric ? <Text style={styles.metric}> · {metric}</Text> : null}</Text><Ionicons name="chevron-forward" size={18} color={glass.glassPalette.cyan} /></View>
      </Animated.View>
    </Pressable>
  );
}
...
function createStyles(glass: RuntimeGlassTheme) {
  return StyleSheet.create({
    card: { backgroundColor: glass.glassDepth.glass, borderRadius: 16, padding: 16, marginVertical: 6, borderWidth: 1, borderColor: glass.glassSurface.border, shadowOffset: { width: 0, height: 0 }, shadowRadius: 12, elevation: 4 },
    header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 12 },
    titleContainer: { flex: 1 },
    name: { color: glass.glassSurface.textPrimary, fontSize: 16, fontWeight: "700", letterSpacing: 0.5 },
    role: { color: glass.glassSurface.textSecondary, fontSize: 12, marginTop: 2 },
    statusDot: { width: 10, height: 10, borderRadius: 5, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 6 },
    footer: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", borderTopWidth: 1, borderTopColor: glass.glassSurface.border, paddingTop: 10 },
    statusText: { fontSize: 11, fontWeight: "700", letterSpacing: 0.5 },
    metric: { color: glass.glassSurface.textSecondary, fontSize: 11 },
  });
}
