...
export default function MemoryScreen() {
  ...
  return (
    <DrawerScreen kicker="LANGZEIT-GEDÄCHTNIS" title="Gedächtnis">
      <DrawerCard accent={`${cyber.purple}66`}>
      ...
      {projectsQuery.isLoading ? (
        <View style={styles.centerRow}><ActivityIndicator color={cyber.cyan} /><Text style={styles.muted}>Projekte werden geladen …</Text></View>
        ...
        <>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={[styles.projectCard, { borderColor: `${STATUS_ACCENTS[item.status] ?? cyber.cyan}55` }]}>
                <View style={styles.projectHeader}>
                  <Text style={styles.projectName} numberOfLines={1}>{item.name}</Text>
                  <View style={[styles.statusChip, { backgroundColor: `${STATUS_ACCENTS[item.status] ?? cyber.cyan}22`, borderColor: STATUS_ACCENTS[item.status] ?? cyber.cyan }]}>
                    <Text style={[styles.statusText, { color: STATUS_ACCENTS[item.status] ?? cyber.cyan }]}>{PROJECT_STATUS_LABELS[item.status as ProjectStatus] ?? item.status}</Text>
                  </View>
                </View>
...
function createStyles() {
  return StyleSheet.create({
    ...
    muted: { color: cyber.textDim, fontSize: 12, lineHeight: 18 },
    sectionLabel: { color: cyber.textDim, fontSize: 10, fontWeight: "800", letterSpacing: 1.4, marginBottom: 10 },
    projectCard: { backgroundColor: cyber.surface, borderRadius: 14, borderWidth: 1, marginBottom: 10, padding: 14 },
    projectHeader: { alignItems: "flex-start", flexDirection: "row", gap: 8 },
    projectName: { color: cyber.text, flex: 1, fontSize: 14, fontWeight: "800" },
    ...
    projectDescription: { color: cyber.textMuted, fontSize: 11, lineHeight: 16, marginTop: 6 },
    projectMeta: { color: cyber.textDim, fontSize: 10, marginTop: 5 },
