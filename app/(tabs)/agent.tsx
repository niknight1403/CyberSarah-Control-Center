  ...
  const submitPrompt = async () => {
    ...
    try {
      const enabledSkills = [
        ...
        .filter(Boolean)
        .join(", ");
      const contextualPrompt = enabledSkills
        ? `${normalizedPrompt}\n\nAktive Skills: ${enabledSkills}`
        : normalizedPrompt;
      const proposal = await requestDevelopmentProposal({
        prompt: contextualPrompt,
        activeFile: selectedFile.remote ? selectedFile.path : undefined,
      });
  ...
  const testConnector = async (connector: ConnectorId) => {
    ...
    try {
      if (connector === "workspace") {
        ...
        await requestDevelopmentProposal({
          prompt: "Verbindungstest: Antworte ausschließlich mit OK.",
        });
      }
      setConnectorTests((current) => ({
        ...current,
                  ...
                  <View style={styles.composerToolbar}>
                    ...
                    >
                      ...
                      <Text style={styles.toolbarButtonText}>
                        {mediaPickerBusy ? "Auswahl …" : "Anhängen"}
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    <View style={styles.toolsMenu}>
                      ...
                      <Text style={styles.toolsHint}>
                        {mediaPickerBusy
                          ...
                          : "Connectoren nutzen die sichere Konfiguration aus den Einstellungen. Tokens werden hier nie angezeigt."}
                      </Text>
                    </View>
                  ) : null}
                  {attachments.length ? (
