...
type ChatMessage = {
  ...
  state?: "ready" | "applying" | "applied" | "error";
};
...
export default function AgentScreen() {
  ...
  const submitPrompt = async () => {
    ...
    setMessages((current) => [...current, { id: userId, role: "user", content: normalizedPrompt }]);
    setPrompt("");
    setChatError("");
    setIsThinking(true);
    try {
      ...
      const content = proposal.changes.length
        ...
        : `${proposal.summary}\n\n${proposal.rationale}`;
      setMessages((current) => [...current, { id: `proposal-${Date.now()}`, role: "agent", content, proposal, state: "ready" }]);
    } catch (error) {
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <>
              ...
              <View style={[styles.readinessCard, readyForChat ? styles.readinessReady : styles.readinessWaiting]}>
                <View style={styles.readinessIcon}><IconSymbol name="sparkles" size={17} color={readyForChat ? "#B9B2FF" : "#F6BA5E"} /></View>
              ...
              <View style={styles.contextRow}>
                <View style={styles.contextChip}><IconSymbol name="doc.text.fill" size={14} color="#8B7CFF" /><Text numberOfLines={1} style={styles.contextText}>{contextLabel}</Text></View>
                <StatusBadge label={readyForChat ? "Kontrolliert" : "Offline"} tone={readyForChat ? "accent" : "warning"} />
              </View>
          ...
          ListFooterComponent={
            <View style={styles.composerCard}>
              <Text style={styles.composerLabel}>NÄCHSTER ENTWICKLUNGSAUFTRAG</Text>
              <TextInput accessibilityLabel="Entwicklungsauftrag für den Agenten" editable={readyForChat && !isThinking} multiline onChangeText={setPrompt} placeholder="Beschreibe eine Änderung, einen Fehler oder ein Refactoring …" placeholderTextColor="#708095" style={styles.composerInput} textAlignVertical="top" value={prompt} />
              <PrimaryButton icon="arrow.up.circle.fill" label={isThinking ? "Agent analysiert …" : "Vorschlag erstellen"} onPress={() => void submitPrompt()} disabled={!prompt.trim() || !readyForChat || isThinking} />
              <Text style={styles.composerHint}>Der Agent erzeugt nur einen überprüfbaren Vorschlag. Er commitet oder pusht niemals selbstständig.</Text>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View style={[styles.messageRow, isUser && styles.userMessageRow]}>
                {!isUser ? <View style={styles.agentAvatar}><IconSymbol name="sparkles" size={16} color="#B9B2FF" /></View> : null}
                <View style={[styles.messageBubble, isUser ? styles.userBubble : styles.agentBubble]}>
                  <Text style={[styles.messageRole, isUser && styles.userMessageRole]}>{isUser ? "DU" : item.proposal ? "ÄNDERUNGSVORSCHLAG" : "AGENT"}</Text>
                  <Text style={styles.messageText}>{item.content}</Text>
                  {item.proposal?.changes.length ? <View style={styles.changeList}>{item.proposal.changes.map((change) => <View key={change.path} style={styles.changeRow}><IconSymbol name="doc.text.fill" size={13} color="#8B7CFF" /><View style={styles.changeCopy}><Text style={styles.changePath}>{change.path}</Text><Text numberOfLines={2} style={styles.changeExplanation}>{change.explanation}</Text></View></View>)}</View> : null}
                  {item.proposal?.changes.length && item.state !== "applied" ? <TouchableOpacity accessibilityRole="button" activeOpacity={0.75} disabled={item.state === "applying"} onPress={() => void applyProposal(item.id, item.proposal!)} style={[styles.applyButton, item.state === "applying" && styles.applyButtonDisabled]}><Text style={styles.applyText}>{item.state === "applying" ? "Übernimmt …" : item.state === "error" ? "Erneut übernehmen" : "Vorschlag übernehmen"}</Text><IconSymbol name="arrow.right" size={14} color="#DFF7FF" /></TouchableOpacity> : null}
                  {item.state === "applied" ? <Text style={styles.appliedText}>Übernommen · Commit und Push bleiben in deiner Kontrolle.</Text> : null}
                </View>
              </View>
            );
          }}
...
const styles = StyleSheet.create({
  ...
  readinessCard: { alignItems: "center", borderRadius: 16, borderWidth: 1, flexDirection: "row", gap: 10, marginBottom: 13, padding: 12 },
  readinessReady: { backgroundColor: "#181A2F", borderColor: "#4D477A" },
  readinessWaiting: { backgroundColor: "#211D16", borderColor: "#6A542A" },
  readinessIcon: { alignItems: "center", backgroundColor: "#25233E", borderRadius: 11, height: 35, justifyContent: "center", width: 35 },
  readinessCopy: { flex: 1 },
  readinessTitle: { color: "#EDF4FC", fontSize: 13, fontWeight: "900", marginBottom: 3 },
  readinessText: { color: "#9CAABD", fontSize: 11, lineHeight: 16 },
  contextRow: { alignItems: "center", flexDirection: "row", gap: 8, justifyContent: "space-between", marginBottom: 25 },
  contextChip: { alignItems: "center", backgroundColor: "#171C29", borderColor: "#2A3449", borderRadius: 999, borderWidth: 1, flex: 1, flexDirection: "row", gap: 6, maxWidth: 220, paddingHorizontal: 11, paddingVertical: 8 },
  ...
  agentAvatar: { alignItems: "center", backgroundColor: "#25233E", borderRadius: 13, height: 27, justifyContent: "center", marginTop: 4, width: 27 },
  ...
  changeList: { borderTopColor: "#2A3548", borderTopWidth: 1, marginTop: 11, paddingTop: 4 },
  changeRow: { flexDirection: "row", gap: 7, marginTop: 9 },
  changeCopy: { flex: 1 },
  changePath: { color: "#C9C0FF", fontFamily: "monospace", fontSize: 11, fontWeight: "800" },
  changeExplanation: { color: "#9DACBD", fontSize: 11, lineHeight: 16, marginTop: 2 },
  applyButton: { alignItems: "center", backgroundColor: "#206275", borderColor: "#52D8FF", borderRadius: 10, borderWidth: 1, flexDirection: "row", gap: 5, justifyContent: "center", marginTop: 13, paddingVertical: 10 },
  applyButtonDisabled: { opacity: 0.55 },
  applyText: { color: "#E6FAFF", fontSize: 12, fontWeight: "900" },
  appliedText: { color: "#78DFA8", fontSize: 11, fontWeight: "800", marginTop: 12 },
  composerCard: { backgroundColor: "#121823", borderColor: "#2B3850", borderRadius: 20, borderWidth: 1, marginTop: 16, padding: 15 },
  composerLabel: { color: "#75859B", fontSize: 10, fontWeight: "900", letterSpacing: 1.2, marginBottom: 9 },
  composerInput: { color: "#EEF4FC", fontSize: 14, lineHeight: 20, maxHeight: 120, minHeight: 88, padding: 0 },
  composerHint: { color: "#8293A8", fontSize: 11, lineHeight: 16, marginTop: 10 },
