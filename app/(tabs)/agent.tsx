...
export default function AgentScreen() {
  ...
  const removeAttachment = (id: string) => {
    ...
    setLoadingPreviewIds((current) => { const next = { ...current }; delete next[id]; return next; });
  };
  ...
  const testConnector = async (connector: ConnectorId) => {
    setConnectorTests((current) => ({ ...current, [connector]: { status: "testing", message: "Prüfung läuft …" } }));
    try {
      ...
      setConnectorTests((current) => ({ ...current, [connector]: { status: "error", message: "Verbindung konnte nicht bestätigt werden" } }));
    }
  };
    ...
    <ScreenContainer className="px-5" edges={["top", "left", "right", "bottom"]}>
      <StudioErrorBoundary section="Agent"><KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.flex}>
        <FlatList
          ...
          ListFooterComponent={<><View style={styles.composerCard}><View style={styles.composerToolbar}><TouchableOpacity accessibilityLabel="Anhänge hinzufügen" accessibilityRole="button" disabled={mediaPickerBusy} activeOpacity={0.75} onPress={() => { setShowAttachMenu((current) => !current); setShowToolsMenu(false); }} style={[styles.plusButton, mediaPickerBusy && styles.applyButtonDisabled]}>{mediaPickerBusy ? <ActivityIndicator color="#52D8FF" size="small" /> : <Text style={styles.plusButtonText}>＋</Text>}<Text style={styles.toolbarButtonText}>{mediaPickerBusy ? "Auswahl …" : "Projektdateien"}</Text></TouchableOpacity><TouchableOpacity accessibilityLabel="Skills und Connectoren öffnen" accessibilityRole="button" activeOpacity={0.75} onPress={() => { setShowToolsMenu((current) => !current); setShowAttachMenu(false); }} style={styles.toolsButton}><Text style={styles.toolsButtonText}>✦ Skills & Connectoren</Text></TouchableOpacity></View>{showAttachMenu ? <View style={styles.attachMenu}>{attachmentKinds.map(({ kind, label }) => <TouchableOpacity accessibilityRole="button" disabled={mediaPickerBusy} key={kind} onPress={() => kind === "datei" ? void pickFiles() : void pickMedia(kind)} style={[styles.attachOption, mediaPickerBusy && styles.applyButtonDisabled]}><Text style={styles.attachOptionIcon}>{kind === "datei" ? "＋" : kind === "foto" ? "▧" : "▶"}</Text><Text style={styles.attachOptionText}>{label} auswählen</Text></TouchableOpacity>)}</View> : null}{showToolsMenu ? <View style={styles.toolsMenu}><Text style={styles.toolsMenuLabel}>SKILLS & CONNECTOREN · {enabledSkillCount(skillPreferences)} SKILLS · {enabledConnectorCount(connectorPreferences)} CONNECTOREN</Text><View style={styles.toolStatusRow}><Text style={styles.toolName}>Agent-Vorschläge</Text><TouchableOpacity accessibilityRole="switch" accessibilityState={{ checked: skillPreferences.agent }} onPress={() => updateSkillPreference("agent")} style={[styles.skillToggle, skillPreferences.agent && styles.skillToggleOn]}><View style={[styles.skillToggleKnob, skillPreferences.agent && styles.skillToggleKnobOn]} /></TouchableOpacity></View><View style={styles.toolStatusRow}><Text style={styles.toolName}>Code-Diff-Prüfung</Text><TouchableOpacity accessibilityRole="switch" accessibilityState={{ checked: skillPreferences.diff }} onPress={() => updateSkillPreference("diff")} style={[styles.skillToggle, skillPreferences.diff && styles.skillToggleOn]}><View style={[styles.skillToggleKnob, skillPreferences.diff && styles.skillToggleKnobOn]} /></TouchableOpacity></View><View style={styles.toolStatusRow}><Text style={styles.toolName}>CI-Qualitätsprüfung</Text><TouchableOpacity accessibilityRole="switch" accessibilityState={{ checked: skillPreferences.quality }} onPress={() => updateSkillPreference("quality")} style={[styles.skillToggle, skillPreferences.quality && styles.skillToggleOn]}><View style={[styles.skillToggleKnob, skillPreferences.quality && styles.skillToggleKnobOn]} /></TouchableOpacity></View><View style={styles.toolStatusRow}><View style={styles.toolNameBlock}><Text style={styles.toolName}>Workspace-Service</Text><Text style={styles.toolDetail}>{settings.hasServiceAccessToken ? "Zugangsdaten hinterlegt" : "Konfiguration fehlt"}</Text></View><View style={styles.connectorActions}><TouchableOpacity accessibilityRole="switch" accessibilityState={{ checked: connectorPreferences.workspace }} onPress={() => updateConnectorPreference("workspace")} style={[styles.skillToggle, connectorPreferences.workspace && styles.skillToggleOn]}><View style={[styles.skillToggleKnob, connectorPreferences.workspace && styles.skillToggleKnobOn]} /></TouchableOpacity><TouchableOpacity accessibilityRole="button" disabled={connectorTests.workspace.status === "testing"} onPress={() => void testConnector("workspace")} style={styles.testButton}><Text style={styles.testButtonText}>{connectorTests.workspace.status === "testing" ? "Prüfe …" : "Testen"}</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" onPress={() => router.push("/settings")} style={styles.configureButton}><Text style={styles.configureButtonText}>Verwalten</Text></TouchableOpacity></View><Text style={styles.connectorTestText}>{connectorTests.workspace.message ?? "Noch nicht geprüft"}</Text></View><View style={styles.toolStatusRow}><View style={styles.toolNameBlock}><Text style={styles.toolName}>GitHub-Connector</Text><Text style={styles.toolDetail}>{settings.hasGitHubToken ? "Token sicher hinterlegt" : "Token nicht hinterlegt"}</Text></View><View style={styles.connectorActions}><TouchableOpacity accessibilityRole="switch" accessibilityState={{ checked: connectorPreferences.github }} onPress={() => updateConnectorPreference("github")} style={[styles.skillToggle, connectorPreferences.github && styles.skillToggleOn]}><View style={[styles.skillToggleKnob, connectorPreferences.github && styles.skillToggleKnobOn]} /></TouchableOpacity><TouchableOpacity accessibilityRole="button" disabled={connectorTests.github.status === "testing"} onPress={() => void testConnector("github")} style={styles.testButton}><Text style={styles.testButtonText}>{connectorTests.github.status === "testing" ? "Prüfe …" : "Testen"}</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" onPress={() => router.push("/settings")} style={styles.configureButton}><Text style={styles.configureButtonText}>Verwalten</Text></TouchableOpacity></View><Text style={styles.connectorTestText}>{connectorTests.github.message ?? "Noch nicht geprüft"}</Text></View><View style={styles.toolStatusRow}><View style={styles.toolNameBlock}><Text style={styles.toolName}>KI-Provider</Text><Text style={styles.toolDetail}>{providerLabel}</Text></View><View style={styles.connectorActions}><TouchableOpacity accessibilityRole="switch" accessibilityState={{ checked: connectorPreferences.provider }} onPress={() => updateConnectorPreference("provider")} style={[styles.skillToggle, connectorPreferences.provider && styles.skillToggleOn]}><View style={[styles.skillToggleKnob, connectorPreferences.provider && styles.skillToggleKnobOn]} /></TouchableOpacity><TouchableOpacity accessibilityRole="button" disabled={connectorTests.provider.status === "testing"} onPress={() => void testConnector("provider")} style={styles.testButton}><Text style={styles.testButtonText}>{connectorTests.provider.status === "testing" ? "Prüfe …" : "Testen"}</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" onPress={() => router.push("/settings")} style={styles.configureButton}><Text style={styles.configureButtonText}>Verwalten</Text></TouchableOpacity></View><Text style={styles.connectorTestText}>{connectorTests.provider.message ?? "Noch nicht geprüft"}</Text></View><Text style={styles.toolsHint}>{mediaPickerBusy ? "Auswahl wird geöffnet …" : "Projektdateien werden lokal begrenzt gelesen; Tokens und Binärdaten werden nicht an den Agenten übergeben."}</Text></View> : null}{attachments.length ? <View style={styles.attachmentList}>{attachments.map((attachment) => { const isPhoto = attachment.kind === "foto"; const isLoading = Boolean(loadingPreviewIds[attachment.id]); return <View key={attachment.id} style={styles.attachmentPreviewCard}>{isPhoto ? <View style={styles.previewImageFrame}>{isLoading ? <View style={styles.previewLoading}><ActivityIndicator color="#52D8FF" size="small" /><Text style={styles.previewLoadingText}>Vorschau wird geladen …</Text></View> : null}<ExpoImage accessibilityLabel={`Vorschau von ${attachment.name}`} cachePolicy="disk" contentFit="cover" onError={() => setPreviewLoading(attachment.id, false)} onLoadEnd={() => setPreviewLoading(attachment.id, false)} onLoadStart={() => setPreviewLoading(attachment.id, true)} source={{ uri: attachment.uri }} style={styles.previewImage} transition={200} /></View> : <View style={styles.previewFallback}><Text style={styles.previewFallbackIcon}>{attachment.kind === "video" ? "▶" : "▤"}</Text><Text style={styles.previewFallbackKind}>{attachment.kind === "video" ? "VIDEO" : "DATEI"}</Text></View>}<View style={styles.previewMeta}><Text numberOfLines={1} style={styles.attachmentName}>{attachment.name}</Text><Text style={styles.previewKind}>{attachment.kind} · bereit zur Überprüfung</Text></View><TouchableOpacity accessibilityLabel={`${attachment.name} entfernen`} accessibilityRole="button" onPress={() => removeAttachment(attachment.id)} style={styles.removeAttachment}><Text style={styles.removeAttachmentText}>×</Text></TouchableOpacity></View>; })}</View> : null}<Text style={styles.composerLabel}>NÄCHSTER ENTWICKLUNGSAUFTRAG</Text><TextInput accessibilityLabel="Entwicklungsauftrag für den Agenten" editable={readyForChat && !isThinking} multiline onChangeText={setPrompt} placeholder="Beschreibe eine Änderung, einen Fehler oder ein Refactoring …" placeholderTextColor="#708095" style={styles.composerInput} textAlignVertical="top" value={prompt} /><PrimaryButton icon="arrow.up.circle.fill" label={isThinking ? "Agent analysiert …" : "Vorschlag erstellen"} onPress={() => void submitPrompt()} disabled={(!prompt.trim() && attachments.length === 0) || !readyForChat || isThinking} /><Text style={styles.composerHint}>Der Agent erzeugt nur einen überprüfbaren Vorschlag. Er commitet oder pusht niemals selbstständig.</Text>{chatError ? <Text style={styles.chatError}>{chatError}</Text> : null}</View><View style={styles.backupCard}><View style={styles.backupTitleRow}><IconSymbol name="lock.fill" size={16} color="#F2C979" /><Text style={styles.backupTitle}>VERSCHLÜSSELTES SUPPORT-BACKUP</Text></View><Text style={styles.backupHint}>Erzeugt eine passwortgeschützte Datei mit dem begrenzten Chat-Verlauf. Zugangsdaten und vollständige Datei-Inhalte sind ausgeschlossen. Das Passwort wird nicht gespeichert.</Text><TextInput accessibilityLabel="Passwort für verschlüsseltes Support-Backup" autoCapitalize="none" onChangeText={setBackupPassword} placeholder="Mindestens 12 Zeichen" placeholderTextColor="#708095" secureTextEntry style={styles.backupInput} value={backupPassword} /><TextInput accessibilityLabel="Passwort für verschlüsseltes Support-Backup wiederholen" autoCapitalize="none" onChangeText={setBackupPasswordRepeat} placeholder="Passwort wiederholen" placeholderTextColor="#708095" secureTextEntry style={styles.backupInput} value={backupPasswordRepeat} /><PrimaryButton icon="square.and.arrow.up" label={backupState === "exporting" ? "Backup wird verschlüsselt …" : "Freigabe prüfen und teilen"} onPress={confirmBackupExport} disabled={backupState === "exporting" || !historyLoaded || !isValidSupportBackupPassword(backupPassword) || backupPassword !== backupPasswordRepeat} />{backupMessage ? <Text style={backupState === "shared" ? styles.backupSuccess : styles.backupError}>{backupMessage}</Text> : null}{backupIntegrityVerified && backupPreview ? <View style={styles.backupVerification}><Text style={styles.backupVerificationTitle}>Integrität bestätigt · Wiederherstellungsvorschau</Text><Text style={styles.backupVerificationText}>{backupPreview.messageCount} Chat-Nachricht(en) im Backup; es werden nur die letzten zwei Ausschnitte angezeigt.</Text>{backupPreview.excerpts.map((excerpt, index) => <Text key={`${index}-${excerpt}`} numberOfLines={2} style={styles.backupPreviewExcerpt}>„{excerpt}“</Text>)}</View> : null}</View></>}
          renderItem={({ item }) => {
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, content: { paddingBottom: 20 }, chatHero: { alignItems: "center", backgroundColor: "#171827", borderColor: "#37335C", borderRadius: 20, borderWidth: 1, flexDirection: "row", gap: 12, marginBottom: 10, padding: 16 }, chatHeroIcon: { alignItems: "center", backgroundColor: "#302B5A", borderRadius: 17, height: 44, justifyContent: "center", width: 44 }, chatHeroCopy: { flex: 1 }, chatHeroTitle: { color: "#F5F2FF", fontSize: 16, fontWeight: "900", marginBottom: 4 }, chatHeroText: { color: "#A7A5BC", fontSize: 11, lineHeight: 16 }, promptChips: { flexDirection: "row", flexWrap: "wrap", gap: 7, marginBottom: 16 }, promptChip: { backgroundColor: "#121827", borderColor: "#2E3C55", borderRadius: 999, borderWidth: 1, minHeight: 36, justifyContent: "center", paddingHorizontal: 12 }, promptChipText: { color: "#B8C7DB", fontSize: 10, fontWeight: "800" },
