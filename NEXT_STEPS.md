...
Stand: 2026-09-14 (Release **v1.3.6**; Agenten-Fundament + Elite-Override + Tech-Scanner — siehe Sprint 94/95)
...
| 92 | Review- und Betriebswacht vollautonom statt externer Anmeldungen | **Erledigt (14.09.):** KI-Code-Review via PR-Agent + bestehendem GEMINI_API_KEY (auto describe/review/improve, deutsch, Bot-PRs übersprungen); Uptime-Wächter als Actions-Cron alle 30 Min gegen app.cybersarah-ki.com (3 Versuche, Issue-Alarm ohne Duplikate + Discord-Webhook, Auto-Close bei Erholung); Coverage-PR-Kommentare via eigenem Skript statt Codecov (Gesamt + schwächste Dateien). Keine externen Konten, keine neuen Secrets. Bericht: `docs/SPRINT_92_REVIEW_UND_BETRIEBSWACHT.md`. |
...
| 93 | Sub-Agenten-Vollausbau des Master-Agenten-Daten-Hubs | **Erledigt (14.09.):** Analytics (GA4 Data API, 7 Tage, Konversionsrate), CRM (HubSpot-Kontakte + Salesforce-Status), Content (TikTok/Instagram-Status) und KI-Dienste (Perplexity/ElevenLabs/Symphony-Key-Status) als key-gated Sub-Agenten mit Retry-Backoff; Kraken-Fallback für Krypto-Kurse bei Binance-Ausfall; drei neue Nur-Lese-Business-Tools im Chat-Master-Agenten (datengetriebene Registry); Dashboard-Kacheln für GA4/CRM/Content mit ehrlichen Nicht-Konfiguriert-Zuständen. 625 Tests grün. Bericht: `docs/SPRINT_93_SUB_AGENTEN_EXPANSION.md`. |
| 94 | Langzeit-Gedächtnis des Master-Agenten & Guardrail-Schärfung | **Erledigt (14.09.):** `agentLearnings`-Tabelle (Migration 0004, Auto-Migrate im Render-Deploy), reine Retrieval-Logik (Klassifikation, Keyword-Extraktion, Relevanz-Ranking, Top-3-Prompt-Injektion), neues Agent-Tool `save_learning`, Auto-Learning nach werkzeuglastigen Turns (write/commit/push), Guardrails auf alle 6 Business-Tools + explizite operationelle Grenzen geschärft. Damit sind alle vier Komponenten der Produktvision (Dispatcher, Identität, Werkzeuge, Gedächtnis) + Generative UI vollständig. 633 Tests grün. Bericht: `docs/SPRINT_94_LANGZEIT_GEDAECHTNIS.md`. |
| 95 | Administrator-Elite-Override & Autonomer Tech-Scanner | **Erledigt (14.09.):** Elite-Routing (`adminPriority` +2 für Fähigkeits-Top-Provider an beiden Routing-Stellen; Quota-Bypass und Auto-Route für Admins bestanden bereits), Tech-Scanner als täglicher Workflow (05:30 UTC): GitHub-Search/HuggingFace/npm-Rohdaten → gewichtete Relevanz-Bewertung (24 Signale) → `docs/research/LATEST_TECH_SCAN.md` + Issue-Update, `[skip ci]`-Commit, kein externer Key nötig; erster Lauf: 86 Rohdaten → 9 relevante Funde. 639 Tests grün. Bericht: `docs/SPRINT_95_ADMIN_ELITE_UND_TECH_SCANNER.md`. |
