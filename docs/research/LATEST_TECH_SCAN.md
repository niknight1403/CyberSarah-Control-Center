# 📡 Tech-Scan 2026-09-18

9 relevante Funde (Mindest-Relevanz 3, max. 4 je Bereich):

- **[GitHub] [karacca/moodstone](https://github.com/karacca/moodstone)** (39★) — Relevanz 14, Bereich: Mobile-Stack/Agenten-Kern/DevOps
  Animated, procedurally generated agent avatars for React Native. Topics: agent-ui, ai-agent, ai-agents, android, animated-svg, animation, avatar, avatars, expo, generative-art, ios, llm, procedural-generation, react, rea

- **[GitHub] [Akxan/google-seo-mcp](https://github.com/Akxan/google-seo-mcp)** (38★) — Relevanz 9, Bereich: Agenten-Kern/DevOps
  SEO & GEO MCP server: Google Search Console, GA4, PageSpeed Insights, structured data, llms.txt, AI crawler checks, WordPress (Yoast, WP-CLI over SSH), GitHub (in-place edits, images) and Gmail attachments as 96 tools fo

- **[GitHub] [MingyiSecLab/Atlas](https://github.com/MingyiSecLab/Atlas)** (10★) — Relevanz 9, Bereich: Agenten-Kern/DevOps
  Open-source local AI Agent desktop app for authorized security assessment — Electron + Mastra-powered runtime, visual pentest workflow, Kali sandbox, multi-provider LLM routing. 本地 AI 安全评估智能体桌面应用 Topics: ai-agent, ai-sec

- **[GitHub] [Achref23illi/ui-craft](https://github.com/Achref23illi/ui-craft)** (5★) — Relevanz 8, Bereich: Mobile-Stack/Agenten-Kern
  Agent skill that makes mobile UI look like a shipped app: rules with real numbers, screen recipes and a fast screenshot review, from 2,600 screens of 44 iOS apps. Claude Code, Codex, Cursor, Gemini CLI. By Achref Arabi. 

- **[GitHub] [Appllama/liquid-glass-screens](https://github.com/Appllama/liquid-glass-screens)** (290★) — Relevanz 8, Bereich: Mobile-Stack/DevOps
  Explore liquid-glass welcome screens with floating stickers and interactive swipe animations. Topics: animation, expo, expo-router, glassmorphism, ios, liquid-glass, mobile-development, mobile-ui, onboarding, react, reac

- **[GitHub] [Innei/lody-ios](https://github.com/Innei/lody-ios)** (140★) — Relevanz 8, Bereich: Mobile-Stack/Agenten-Kern
  Independent iOS client for Lody — native chat, sessions, and coding-agent collaboration on iPhone Topics: agent, coding-agent, collaboration, expo, ios, iphone, local-first, react-native, swift

- **[GitHub] [Albertchamberlain/Awesome-OKF](https://github.com/Albertchamberlain/Awesome-OKF)** (100★) — Relevanz 7, Bereich: Agenten-Kern
  OKF (Open Knowledge Format) — curated catalog of tools, plugins, skills, proposals, and docs for agent-friendly knowledge. YAML-driven, agent-searchable, MCP-ready. Topics: agent-memory, agent-skills, ai-agents, ai-memor

- **[GitHub] [friday-memory/friday](https://github.com/friday-memory/friday)** (33★) — Relevanz 7, Bereich: Agenten-Kern
  ⚡ The open-source persistent cognitive memory layer for AI coding agents. Stop your Cursor, Claude & Copilot from having amnesia. Topics: ai-agents, ai-memory, antigravity, antigravity-cli, antigravity-ide, claude-code, 

- **[npm] [drizzle-orm 0.45.2](https://www.npmjs.com/package/drizzle-orm)** (v0.45.2) — Relevanz 6, Bereich: Backend
  Aktuellste npm-Version. Lokal installiert: ^0.45.2. Drizzle ORM package for SQL databases
