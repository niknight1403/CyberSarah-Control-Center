 ...
 * @param {string} [env.groqApiKey]
 * @param {string} [env.openrouterApiKey]
...
export function buildServiceEnv({
  ...
  const required = {
    ...
    // Session-Tokens (JWT) tragen ENV.appId im Payload; verifySession lehnt
    // leere appId ab. Ohne diese Variable stellt der Server Tokens aus, die
    // er selbst verwirft -> Login/Registrierung scheitert an 401.
    // Wert MUSS deploy-uebergreifend stabil bleiben (Bestandteil jedes Tokens).
    VITE_APP_ID: "cybersarah-control-center",
  };
  ...
  if (geminiApiKey) optional.AI_GEMINI_API_KEY = geminiApiKey;
  // Sprint 108 (Zero-Cost-Router): Kostenlose Provider — der Server
  // (development-chat.ts) liest AI_GROQ_API_KEY/GROQ_API_KEY bzw.
  // AI_OPENROUTER_API_KEY/OPENROUTER_API_KEY. Ohne Key bleibt der
  // Provider einfach inaktiv (optional, kein Deploy-Fehler).
  if (groqApiKey) optional.AI_GROQ_API_KEY = groqApiKey;
  if (openrouterApiKey) optional.AI_OPENROUTER_API_KEY = openrouterApiKey;
...
export function buildWorkspaceEnv({
  ...
  // Sprint 108 (Zero-Cost-Router): Der Workspace-Service
  // (workspace-service/src/index.js) liest GROQ_API_KEY bzw.
  // OPENROUTER_API_KEY — schlichten Namen, gleiche Keys wie im Chat-Routing.
  if (groqApiKey) map.GROQ_API_KEY = groqApiKey;
  if (openrouterApiKey) map.OPENROUTER_API_KEY = openrouterApiKey;
...
/**
 * Sprint-85-Follow-up (Deploy-Fix): Eindeutige Erkennung eines NEUEN Deploys.
 *
 * Bug: waitForLive suchte frueher "den ersten Deploy mit anderer ID als der
 * letzte" — das konnte ein BELIEBIGER aelterer Deploy aus den letzten 5 sein,
 * waehrend list[0] (der weiterhin live alte Deploy) sofort als Erfolg
 * galt. Der Health-Check lief dann gegen die ALTE Instanz → false gruen.
 *
 * Fix: Vor dem ENV-PUT wird ein Snapshot der bekannten Deploy-IDs gezogen;
 * "frisch" ist der neueste Listeneintrag, dessen ID NICHT im Snapshot steht.
 */
export function pickFreshDeploy(deploys, beforeIds) {
  const seen = beforeIds instanceof Set ? beforeIds : new Set(beforeIds ?? []);
  for (const entry of deploys ?? []) {
    const deploy = entry?.deploy ?? entry;
    if (deploy?.id && !seen.has(deploy.id)) return deploy;
  }
  return null;
}
